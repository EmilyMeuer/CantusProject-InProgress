...
class CorruptInput
{
  /*
   ...
   TODO:
     ...
     - Better implementation of input numbers
   */
  ...
  float                  sensitivity;  // amplitude below which 
  ShortFrameSegmenter    sfs;          //   adjustedFreq will not be reset
  ShortFrameSegmenter[]  sfsArray;
  int                    waitUntil;

  CorruptInput(int numInputs)
  {
...
/*      
      if (i < 4) {
        ...
        uGenArray[i]  = ac.getAudioInput(new int[] {(i + 5)});
      } // else
*/
      uGenArray[i]  = ac.getAudioInput(new int[] {(i + 1)});
//      println("uGenArray[" + i +"] = " + uGenArray[i]);
 ...
 //     g.addInput(uGenArray[i]);
  ...
  float getAverageAmp(int firstInput, int lastInput)
  {
    ...
    int[]  inputsToAverage  = new int[lastInput - firstInput + 1];
    for (int i = 1; i <= inputsToAverage.length; i++)
    {
      inputsToAverage[i]  = curInput;
      curInput++;
    } // for
 ...
 */
class CorruptFrequencyEMM extends FeatureExtractor<Float, float[]> {
   ...
   */
  public CorruptFrequencyEMM(float sampleRate) {
    bufferSize = -1;
