...
void setup()
{
  ...
  //Creating a new Input object for 2 mics:
...
void draw()
{
  ...
  {
    if(j % 2 == 0)  {
      amp  = leftInput.getAmplitude();
    } else {
      amp  = rightInput.getAmplitude();
    } // if - getting amplitude from either the right or the left input
    
    levels[j]  = (int)Math.floor(amp * volumeAdjust);
  } // for - find amp levels
  ...
  float pointillizeFive = map((Math.min(leftInput.getAmplitude(), 2)), 0, 30, smallPoint, largePoint);
