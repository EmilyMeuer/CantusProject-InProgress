...
*/
  ...
  float  redVol  = leftInput.getAmplitude() / volAdjust;
...
class LeadBall {
  ...
  void move () {
    xLead = xLead - speed*leftInput.getAmplitude() * amplify;      // amplify is a fraction
    ...
    if (xLead < 0) {
      ...
      yLead = (height * 0.9) - leftInput.getAdjustedFundAsHz();
...
class Ball {
  ...
  void move() {
    ...
    if (ballNumber != 0) {
      ...
      if ((x < 1)&&(x>-1)) {
        x = width;
        y = height*0.9-leftInput.getAdjustedFundAsHz();
