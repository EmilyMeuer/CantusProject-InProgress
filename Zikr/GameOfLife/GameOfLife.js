...
function createFirstGeneration ( array2d )
{
    ...
    evenOrOdd = array2d.length % 2 + 1;
    i = array2d.length/2 - (1/evenOrOdd);
    j = array2d[i].length/2 - (1/evenOrOdd);
    
    array2d[i-1][j-1]   = getLiveValue();
    array2d[i-1][j+1]   = getLiveValue();
    array2d[i][j]   = getLiveValue();
    array2d[i+1][j-1]   = getLiveValue();
    array2d[i+1][j+1]   = getLiveValue();
/*
    // Every-other "checkerboard" pattern:
	for (i = 0; i<array2d.length; i++)
	{
		for (j = 0; j<array2d[i].length; j++)
		{
