/*
  ...
  * Emily Meuer
  *
  *  Edits to use multiple live inputs.
 */
...
//  InputPitch bass1 = ins.get(0);
  float rfvol = input.getAmplitude(1) / 500;  //'volume' of bass1, used for columns RED and FUCHSIA
  float rfpit = input.getAdjustedFund(1);  //'pitch' of bass1, used for columns RED and FUCHSIA
  
//  InputPitch bass2 = ins.get(1);
  float opvol = input.getAmplitude(2) / 100;
  float oppit = input.getAdjustedFundAsHz(2);
  
//  InputPitch tenor1 = ins.get(2);
  float yivol = input.getAmplitude(3) / 100;
  float yipit = input.getAdjustedFundAsHz(3);
  
//  InputPitch tenor2 = ins.get(3);
  float gpvol = input.getAmplitude(4) / 100;
  float gppit = input.getAdjustedFundAsHz(4);
  
  float  vol5  = input.getAmplitude(5);
  float  vol6  = input.getAmplitude(6);
  println("vol6 = " + vol5);
