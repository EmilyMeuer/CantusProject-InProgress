...
// Calibrate:
int  volumeAdjust  = 1000;   // not currently used.
int  fundAdjust    = 8;      // divide pitch by this (will probably need to make it smaller to account for their low voices).
...
void draw()
{
  ...
  fourLevel  = (int)Math.floor(input.getAmplitude(4));
  // fiveLevel  = (int)Math.floor(input.getAmplitude() * 1500);
  sixLevel  = (int)Math.floor(input.getAmplitude(1));
  sevenLevel  = (int)Math.floor(input.getAmplitude(2));
...
/*
  ...
  threeLevel  = (int)Math.floor(input.getAdjustedFund(3) / fundAdjust);
  fourLevel  = (int)Math.floor(input.getAdjustedFund(4) / fundAdjust);
  // fiveLevel  = (int)Math.floor(input.getAdjustedFund() * 1500);
  sixLevel  = (int)Math.floor(input.getAdjustedFund(1) / fundAdjust);
  sevenLevel  = (int)Math.floor(input.getAdjustedFund(2) / fundAdjust);
